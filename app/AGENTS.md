...
- The user selected the "Sơn mài ánh trăng" visual direction. Keep the product distinctly Vietnamese through lacquer-black surfaces, antique gold leaf, vermilion accents, moon/cloud/lotus motifs, and original Vietnamese character art.
- Reserve the richest illustration and ornament for role reveal and ceremonial moments. Host tools should be calmer, denser, and faster to scan while retaining the same palette and typography.
- The core prototype journey is mobile-first: create room → lobby → host night cockpit → night summary → dawn/discussion → voting → round result → next night, plus join room → reveal role → waiting/day state.
- Keep game automation assistive rather than authoritative: the app records targets, outcomes, votes, and narration prompts, while the group still discusses and votes together in person.
- Room creation happens only after the host selects the player count and a complete role deck. The setup UI must keep an always-visible count of total cards, ability roles, Villagers, and faction balance, and block room creation while cards are missing or over capacity.
- Rooms do not have user-entered names. Creating a room generates a random four-digit numeric room ID, shown in the Host header's top-right corner; players join by entering that ID and their display name only.
- The Host lobby does not repeat invitation instructions below the header. Show joined players as a compact two-column grid: one colored block per player, with no ready checkmark or per-player readiness subtitle.
- Only Dân Làng is described as having no ability. Every other initial role is an ability role, including active target selection, passive effects, and death/vote triggers.
- Role-card artwork uses a shared 1024×1536 layer system documented in `CARD_LAYER_SYSTEM.md`: shared background → shared halo/frame → role environment → role subject → focus object → foreground effects → live React UI. Keep focus objects in front of subjects, animate layers independently with `motion/react`, respect reduced-motion, never bake Vietnamese role copy into generated card art, and never duplicate shared layers inside individual role folders.
- The production project is React + TypeScript + Vite with `motion/react`. Standalone HTML files are allowed only as development inspectors or documentation and must never become the product runtime.
- In the host deck builder, represent roles with elongated 2:3 miniature cards on the left rather than circular glyph tiles. Selected cards should feel physically lifted and visually richer, while count controls remain compact and fast to operate. All ten MVP roles now have approved Vietnamese lacquer subject art; render each thumbnail from the shared background and halo plus its own subject layer.
- All role-card surfaces must use the shared role catalog and layered artwork renderer keyed by `roleId`; do not hardcode Tiên Tri copy or asset paths inside individual screens. Tapping a Host deck-picker card opens its artwork, ability and win-condition preview.
- Do not dilute the direction into generic medieval fantasy, purple gaming gradients, glassmorphism, or neon cyberpunk.
